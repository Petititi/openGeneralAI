...
class TrajectoryLogger:
    ...
    def set_questions(self, node_id: str, *, prompt_messages: List[Dict[str, Any]]):
        n = self.nodes[node_id]
        n.prompt_messages = copy.deepcopy(prompt_messages)

    def set_response(self, node_id: str, *, raw_response: str, token_usage: Optional[List[int]] = None, duration_s: Optional[float] = None):
        n = self.nodes[node_id]
        n.raw_response = raw_response
        n.token_usage = token_usage
        n.duration_s = duration_s

    def set_plan(self, node_id: str, *, plan: Dict[str, Any], is_done: bool):
        n = self.nodes[node_id]
        n.plan_snapshot = copy.deepcopy(plan)
        n.is_done = is_done

    def set_tool(self, node_id: str, *, tool_name: str, tool_input: Any, tool_output: Any = None, error: Optional[str] = None):
        n = self.nodes[node_id]
    ...
    def tag(self, node_id: str, **kv):
        self.nodes[node_id].tags.update(kv)
